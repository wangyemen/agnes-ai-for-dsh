...
// ============================================================================
// Generation Records (persisted via a settings namespace)
// ============================================================================
...
/** Agnes 输出 CDN 域名（图片）。 */
const AGNES_IMAGE_URL_PATTERN =
  'https?://cos-platform-outputs\\.agnes-ai\\.cn/[^\\s"\'<>)\\]，。！？；：、]+\\.(?:png|jpg|jpeg|webp|gif)'

/** Agnes 输出 CDN 域名（视频）。 */
const AGNES_VIDEO_URL_PATTERN =
  'https?://cos-platform-outputs\\.agnes-ai\\.cn/[^\\s"\'<>)\\]，。！？；：、]+\\.(?:mp4|webm|mov)'

function findAllUrls(text: string, pattern: string): string[] {
  if (!text) return []
  try {
...
function findAgnesImageUrls(text: string): string[] {
  return findAllUrls(text, AGNES_IMAGE_URL_PATTERN)
}

function findAgnesVideoUrls(text: string): string[] {
...
function collectTextFromResult(result: any): string {
  ...
  // 兜底：直接序列化 result
  if (parts.length === 0) parts.push(stringifyArgs(result))
  return parts.join('\n')
}
 ...
 */
function extractPromptAndModel(argsText: string, resultText: string): {
  prompt: string
  model: string
} {
...
function renderImageVideoResult(_args: any, value: any): Array<{ type: 'text'; text: string }> {
  if (value?.error) return [{ type: 'text', text: `Agnes error: ${value.error}` }]
  if (value?.url) return [{ type: 'text', text: `Result URL: ${value.url}` }]
...
function renderRecordResult(_args: any, value: any): Array<{ type: 'text'; text: string }> {
  if (value?.ok) return [{ type: 'text', text: `Recorded as ${value.id}` }]
  if (value?.error) return [{ type: 'text', text: `Record failed: ${value.error}` }]
  return [{ type: 'text', text: 'No result.' }]
}
...
async function executeVideoGeneration(
  args: any,
  ctx: any,
  onProgress?: (progress: number, status: string) => Promise<void> | void,
): Promise<unknown> {
  const apiKey = await resolveAgnesApiKey(ctx)
  if (!apiKey) {
    return {
...
export function apply(ctx: Context): void {
  ...
  ctx.inject(['tools', 'settings'], (sctx: any) => {
    ...
    async function saveRecords(records: GenerationRecord[]): Promise<void> {
      if (!recordsScope) return
      try {
    ...
    async function addRecord(record: GenerationRecord): Promise<void> {
      const all = loadRecords()
      // 去重：同一 URL 只登记一次（仅当 URL 已设置时）
      if (record.url && all.some((r) => r.url === record.url)) return
      all.unshift(record)
      if (all.length > MAX_RECORDS) all.length = MAX_RECORDS
      await saveRecords(all)
    }

    async function patchRecord(id: string, patch: Partial<GenerationRecord>): Promise<void> {
      const all = loadRecords()
    ...
    try {
      ctx.on('tools/execute', async (exec: any, next: any) => {
        ...
        try {
          ...
          if (
            toolName === AGNES_IMAGE_GENERATION_TOOL.name ||
            toolName === AGNES_VIDEO_GENERATION_TOOL.name
          ) {
            return result
          }

          const argsText = stringifyArgs(
            exec?.args ?? exec?.parameters ?? exec?.input,
          )
          ...
          if (imageUrls.length === 0 && videoUrls.length === 0) return result

          const { prompt, model } = extractPromptAndModel(argsText, resultText)

          // eslint-disable-next-line no-console
          console.error(
            `[agnes-ai-for-dsh] auto-detect from tool "${toolName}": ` +
    ...
    try {
      sctx.tools.register(defineTool({
        ...
        parameters: AGNES_IMAGE_GENERATION_TOOL.parameters as any,
        output: {
          schema: IMAGE_VIDEO_OUTPUT_SCHEMA as any,
        ...
        execute: async (args: any, exec: any) => {
          ...
          try {
            const result: any = await executeImageGeneration(args, exec?.ctx ?? sctx)
            if (result?.url) {
    ...
    try {
      sctx.tools.register(defineTool({
        ...
        parameters: AGNES_VIDEO_GENERATION_TOOL.parameters as any,
        output: {
          schema: IMAGE_VIDEO_OUTPUT_SCHEMA as any,
          render: renderImageVideoResult,
        },
        ...
        execute: async (args: any, exec: any) => {
          ...
          try {
            const result: any = await executeVideoGeneration(
              args,
              exec?.ctx ?? sctx,
              async (progress, status) => {
                await patchRecord(id, { progress, status: status as GenerationRecord['status'] })
              },
            )
            if (result?.url) {
    ...
    try {
      sctx.tools.register(defineTool({
        ...
        parameters: AGNES_RECORD_GENERATION_TOOL.parameters as any,
        output: {
          schema: RECORD_OUTPUT_SCHEMA as any,
